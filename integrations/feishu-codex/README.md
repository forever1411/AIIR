...
- Codex 处理超过 15 分钟时只发送慢响应提醒，不再中断或遗弃该 turn；最终答案到达后会自动补发到原消息；
- 长回复由桥接层按顺序显式分段，第一段回复原消息，后续段落继续发送；超长单行也会硬切分，部分发送失败时不会重发已经成功的段落；
...
如果某轮处理超过 15 分钟，手机端会先收到“任务仍在继续”的提醒，桥接队列随后允许处理 `help`、`status` 等命令；该 turn 完成时，最终答案仍会自动补发。为了避免同一 Codex thread 并发执行多个任务，在上一轮结束前，新研究或写入任务会收到忙碌提示。
