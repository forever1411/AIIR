...
| `CHANGELOG.md`             | 已更新            |
| `DEVELOPMENT_AGENT.md`      | 本地开发规则唯一正式来源       |
| `AGENTS.md`                | Codex 与通用 Agent 兼容入口 |
...
| --------------- | --------------------------------- |
| 操作系统            | Ubuntu 24.04                      |
| 开发执行            | 本地编程代理（Codex、Claude Code 或其他已批准工具） |
| 产品与架构设计         | ChatGPT                           |
| 辅助模型            | Google Gemini                     |
| 版本管理            | Git + GitHub                      |
...
* Ubuntu 本地开发必须有一个可执行代码修改、命令运行和测试验证的编程工具或代理；
* 工具可以是 Codex、Claude Code 或其他满足项目要求的实现；
* 项目流程、角色和验收标准不得绑定某一厂商；
* 统一规则保存在 `DEVELOPMENT_AGENT.md`；工具专属入口只负责跳转，不复制规则。
...
Task
  ↓
Local Coding Agent Implementation
  ↓
Local Coding Agent Self Review
  ↓
